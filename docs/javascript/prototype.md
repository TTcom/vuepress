...
//首先god visual angle
	...
	function father(color,weight){   //fuqin 
		this.grade = 1;
		this.color = color;
		this.weight = weight;
		var abc = "abcd"
	...
	father.prototype.sport = function (){
		console.log('我会跑步')
	}
	
	
	
	function son(color,weight){    //erzi 
		
		father.apply(this,[color,weight])       //通过对象冒充的方式，无法继承通过prototype方式定义的变量和方法;无法继承sport方法
	    this.grade = 2;
...
//	 son.prototype = new father();    //修改son的原型,将son的原型指向new father();
	  
	var dawa = new son('yellow','120');      

//    dawa.sport()             //通过对象冒充的方式，无法继承通过prototype方式定义的变量和方法
	
	
	console.log('dawa',dawa);
	
	
	function grandson(color,weight){    //sunzi 
	
	     // son.apply(this,[color,weight]);
	     son.call(this,'color','weight');
	     this.grade = 3;
...
//	grandson.prototype = new son(); 
	...
	var underson = new grandson('red','320');
	
//	 underson.sport();
		
	console.log(underson)
